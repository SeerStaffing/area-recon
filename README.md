...
> 🤖 **AI agent or new contributor? Start with [CLAUDE.md](CLAUDE.md).** It covers the architecture, data layers & sources, the `index.html` code map, how to run/preview, and deploy.

An interactive map for **scouting an area** — fiber-internet availability, violent crime, and terrain — with address search that resolves down to the **FCC census block**. **County-level fiber and exact block-level address lookup both cover all 50 states** (official FCC data); **West Virginia** carries the extra detail — broadband adoption and violent crime.
...
- **Two map levels**
  ...
  - **County drill-down** — click any state to zoom into its counties; every state shows official FCC fiber and supports exact-block address lookup, and West Virginia adds broadband adoption and crime.
...
- **Address search** — type a full address (e.g. `401 Charles Way, Purgitsville, WV 26852`), with **autocomplete**. The result pin shows the address, elevation, the **2020 census block**, and county fiber stats — and reports whether fiber is available in that **exact census block**, nationwide (each state's block index loads on demand).
...
- **Fiber availability** — [FCC Broadband Data Collection](https://broadbandmap.fcc.gov/) (Dec 2025). Official residential fiber-to-the-premises availability % per county, from the nationwide Summary-by-Geography file; gigabit = ≥ 1000/100 Mbps. The per-state block indexes are derived from each state's location file.
...
County fiber and **exact-block address lookup** are already nationwide — per-state block indexes live in `blocks/`, generated by `scripts/build_state_blocks.ps1` from each state's FCC BDC "Fiber to the Premises" location file (`bdc_<stateFIPS>_FibertothePremises_…`). The remaining **broadband-adoption and county-crime** detail is West Virginia-only; to extend those, add per-county figures for that state.
