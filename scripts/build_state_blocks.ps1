...
foreach ($f in $files) {
  ...
  while (-not $sr.EndOfStream) {
    ...
    [void]$any.Add($blk.Substring(2))
    $dl = 0; [void][int]::TryParse($maxdl, [ref]$dl)
    if ($dl -ge 1000) { [void]$gig.Add($blk.Substring(2)) }
  }
  ...
  Write-Host ("{0}: any={1} nogig={2}" -f $fips, $any.Count, $nogig.Count)
}
Write-Host "Done. Per-state files in $OutDir (wire on-demand <script src> loading in index.html)."
